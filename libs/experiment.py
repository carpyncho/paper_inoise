    ...
    def run(self, train_name):
        ...
        for test_name in self.data.keys():
            ...
            if test_name not in train_name:
                ...
                train_df = train_df[train_df.nbg.isin(self.cfilter)]
                test_df = test_df[test_df.nbg.isin(self.cfilter)]
...
class KFoldExperiment(Experiment):
    ...
    def run(self, subject, nfolds=10):
        ...
        subject_df = subject_df[subject_df.nbg.isin(self.cfilter)]
        ...
        for rst in results:           
            ...
            y_testing = np.hstack([y_testing, rst.y_testing])
            y_testing_real = np.hstack([y_testing_real, rst.y_testing_real])
            del rst
